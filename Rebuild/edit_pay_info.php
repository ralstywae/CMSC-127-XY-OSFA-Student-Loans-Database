<?php
	...
	$MySearchQuery = "SELECT * FROM STUDENT WHERE STUD_NUM = '$num' AND LOAN_TYPE = '$type' AND LOAN_YEAR = '$year' AND LOAN_SEM = '$sem'";
	...
	if($_POST['save'])
	{
		...
		$MyQuery = "UPDATE BAL_HIST SET OR_NUM = $nor_num, DATE_PAID = '$ndate_paid' WHERE (BAL_HIST.STUD_NUM = '$num' AND BAL_HIST.LOAN_TYPE = '$type' AND BAL_HIST.LOAN_YEAR = '$year' AND BAL_HIST.LOAN_SEM = '$sem' AND BAL_HIST.AMT_PAID = $paid AND BAL_HIST.DATE_PAID = '$dpaid' AND BAL_HIST.OR_NUM = $or_num)";
...
<html>
	...
	<body>
		...
		<!-- Payment Form-->
		<h1 class="jumbotron-fluid text-center py-4" style="font-size: 50px"><em>Edit Payment Information</em></h1>
		<div class="container">
			...
			<div class="container">
				<div class="form-group row">
					<div class="form-group col-xs-4 col-md-4 text-center">
						...
						<input class="form-control" name="amt_paid" placeholder="Saved: &#8369;<?php echo $paid; ?>">
					</div>
					<div class="form-group col-xs-4 col-md-4 text-center">
						<label for="date" class="control-label text-center">Date of Payment</label><br>
						<div class="form-inline">
							<input class="form-control col-10" id = "date" name="date_paid" placeholder="Saved: <?php echo $dpaid; ?>" type="text">
							<div class="input-group-addon"><i class="fa fa-calendar"></i></div> 
						</div>
					</div>
					<div class="form-group col-xs-4 col-md-4 text-center">
						<label for="example-number-input" class="control-label text-center">O.R. No.</label>
						<input class="form-control" name="or_num" placeholder="Saved: <?php echo $or_num; ?>"> 
					</div>
				</div>
			</div>
			<div class="row py-2">
